...
import { makePskIndex } from '../../src/clientState.js'
...
export async function testEveryoneCanMessageEveryone (
    ...
    for (const [senderIndex, senderState] of updatedGroups.entries()) {
        ...
        for (const [receiverIndex, receiverGroup] of updatedGroups.entries()) {
            ...
            t.deepEqual(result.message, encodedMessage, `Member ${receiverIndex} should receive message from ${senderIndex}`)
...
export async function cannotMessageAnymore (state: ClientState, impl: CiphersuiteImpl, t: any): Promise<void> {
    let errorThrown = false
    try {
        await createApplicationMessage(state, new TextEncoder().encode('hello'), impl)
    } catch (error) {
        errorThrown = error instanceof UsageError
    }
    t.ok(errorThrown, 'Should throw UsageError when trying to send message after leaving')
}
