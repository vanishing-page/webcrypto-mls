...
import { protect, unprotectPrivateMessage, protectApplicationData, protectProposal } from '../../src/messageProtection.js'
...
import type { AuthenticatedContent } from '../../src/authenticatedContent.js'
...
async function testMessageProtection (t: any, data: MessageProtectionData, impl: CiphersuiteImpl) {
    ...
    await protectThenUnprotectProposalPublic(t, data, gc, impl)

    await publicCommit(t, data, gc, impl)
    await protectThenUnprotectCommitPublic(t, data, gc, impl)

    await proposal(t, data, gc, impl)
    await protectThenUnprotectProposal(t, data, gc, impl)

    await application(t, data, gc, impl)
    await protectThenUnprotectApplication(t, data, gc, impl)

    await commit(t, data, gc, impl)
    await protectThenUnprotectCommit(t, data, gc, impl)
...
async function protectThenUnprotectCommitPublic (t: any, data: MessageProtectionData, gc: GroupContext, impl: CiphersuiteImpl) {
...
async function protectThenUnprotectProposal (t: any, data: MessageProtectionData, gc: GroupContext, impl: CiphersuiteImpl) {
    const p = decodeProposal(hexToBytes(data.proposal), 0)
    if (p === undefined) throw new Error('could not decode proposal')
...
async function protectThenUnprotectApplication (t: any, data: MessageProtectionData, gc: GroupContext, impl: CiphersuiteImpl) {
    const secretTree = await createSecretTree(2, hexToBytes(data.encryption_secret), impl.kdf)
...
async function protectThenUnprotectCommit (t: any, data: MessageProtectionData, gc: GroupContext, impl: CiphersuiteImpl) {
    const c = decodeCommit(hexToBytes(data.commit), 0)
    if (c === undefined) throw new Error('could not decode commit')
