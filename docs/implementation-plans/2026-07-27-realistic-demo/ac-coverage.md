...
| AC10.6 the page states the three disclosures | Two of three. Node `Room says creator-only removal is interface, not cryptography` and `Room says a disconnected member is still in the group`. The placeholder disclosure is gone from the page; see gaps below |
...
Six, each recorded rather than quietly counted as covered. Four are
...
made. Recorded first in US-018 and again in US-020. The page used to say
so, in the placeholder disclosure, which is no longer rendered -- see
AC10.6 below. Covering it would mean storing plaintexts, which is a
design change and is out of this branch's scope.
...
ux") deleted the `What the gaps mean` block from `room.ts`, so nothing
on the page says what a placeholder stands for -- neither that it is a
message said before you were let in or one this browser no longer holds
the keys for, nor that your own messages are shown from a record that
does not outlive the page. `Room says what a placeholder stands for`
kept asserting the deleted markup and failed for two days before being
removed with the rule that styled it. This is uncovered because the
behaviour is gone, not because no test could reach it: the other two
disclosures are Node-covered by exactly the assertion that test made,
and restoring the paragraph would restore its coverage with it. Whether
the copy comes back, and where, is a timeline-work decision that has not
been made.
