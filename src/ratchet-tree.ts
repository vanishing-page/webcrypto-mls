...
import { decodeNodeType, encodeNodeType } from './node-type.js'
...
/**
 * Every unmergedLeaves entry decoded on a parent node must be a valid leaf
 * index for the tree it appears in -- otherwise treemath helpers keyed off
 * that index (directPath/copath/parent) can be handed an out-of-range or
 * negative node index derived from it.
 */
function unmergedLeavesInRange (tree:RatchetTree):boolean {
    const width = leafWidth(tree.length)
    return tree.every((node) => {
        if (node === undefined || node.nodeType !== 'parent') return true
        return node.parent.unmergedLeaves.every((leafIndex) =>
            Number.isInteger(leafIndex) && leafIndex >= 0 && leafIndex < width)
    })
}
...
export function filteredDirectPath (leafIndex:LeafIndex, tree:RatchetTree):NodeIndex[] {
    ...
    return directPath(leafNodeIndex, lWidth).filter((_nodeIndex, n) => resolution(tree, cp[n]!).length !== 0)
}
...
export function getCredentialFromLeafIndex (ratchetTree:RatchetTree, leafIndex:LeafIndex) {
    ...
    if (senderLeafNode === undefined || senderLeafNode.nodeType === 'parent') { throw new ValidationError('Unable to find leafnode for leafIndex') }
...
export function getSignaturePublicKeyFromLeafIndex (ratchetTree:RatchetTree, leafIndex:LeafIndex):Uint8Array {
    ...
    if (leafNode === undefined || leafNode.nodeType === 'parent') { throw new ValidationError('Unable to find leafnode for leafIndex') }
