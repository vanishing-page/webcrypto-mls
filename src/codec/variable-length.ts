...
export function determineLength (data:Uint8Array, offset:number = 0):{ length:number; lengthFieldSize:number } {
    ...
    if (prefix === 0) {
        ...
        const length = ((firstByte & 0b00111111) << 8) | (data[offset + 1] as number)
        if (length < 64) {
            throw new CodecError('Non-minimal 2-byte length encoding')
        }
        return { length, lengthFieldSize: 2 }
    } else if (prefix === 2) {
        ...
        if (length < 16384) {
            throw new CodecError('Non-minimal 4-byte length encoding')
        }
        return { length, lengthFieldSize: 4 }
    } else {
